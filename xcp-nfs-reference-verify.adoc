...
| <<nfs_verify_stats_csv,vérifier -stats>> | Analyse les arborescences source et cible en parallèle et compare les statistiques de l'arborescence. 


| <<nfs_verify_stats_csv,vérifiez -csv>> | Analyse les arborescences source et cible en parallèle et compare les statistiques de l'arborescence. 


| <<nfs_verify_nodata,vérifiez -nodata>> | Ne vérifie pas les données. 


| <<nfs_verify_noatts,vérifiez -noattrs>> | Ne vérifie pas les attributs. 


| <<nfs_verify_nomods,vérifier -nomds >> | Ne vérifie pas les heures de modification du fichier. 


| <<nfs_verify_mtimewindow,vérifier -mtimewindlow  >> | Spécifie la différence de temps de modification acceptable pour la vérification. 


| <<nfs_verify_v_l,vérifier -v >> | Récupère les formats de sortie pour répertorier les différences trouvées. 


| <<nfs_verify_v_l,vérifier -l >> | Récupère les formats de sortie pour répertorier les différences trouvées. 


| <<nfs_verify_nonames,vérifier -non-noms >> | Exclut les noms d'utilisateur et de groupe des listes de fichiers ou des rapports. 


| <<nfs_verify_match,vérifier -match  lt;filtre  gt;  | Traite uniquement les fichiers et répertoires qui correspondent au format. 


| <<nfs_verify_bs,vérifier -bs [k]>  | Spécifie la taille du bloc de lecture/écriture (par défaut : 64 ko). 


| <<nfs_verify_parallel,vérifier -parallèle  >> | Spécifie le nombre maximal de processus de traitement par lots simultanés (par défaut : 7). 


| <<nfs_verify_dircount,vérifier -dircount [k]>  | Spécifie la taille de la demande lors de la lecture des répertoires. 


| <<nfs_verify_noid,Vérifiez -noid >> | Désactive la création d'un index par défaut (par défaut : FALSE). 


| <<nfs_verify_preserveatime,vérifiez -preserve-atime >> | Restaure tous les fichiers à la dernière date d'accès sur la source. 


| <<nfs_verify_s3_insecure,vérifier -s3.insecure >> | Permet d'utiliser HTTP au lieu de HTTPS pour les communications entre compartiments S3. 


| <<nfs_verify_s3_endpoint,vérifier -s3.endpoint  >> | Remplace l'URL du terminal Amazon Web Services (AWS) par défaut par l'URL spécifiée pour la communication du compartiment S3. 


| <<nfs_verify_s3_profile,vérifier -s3.profile  >> | Spécifie un profil à partir du fichier d'informations d'identification AWS pour la communication du compartiment S3. 


| <<nfs_verify_s3_noverify,vérifier -s3.noverify>> | Remplace la vérification par défaut de la certification SSL pour les communications de compartiment S3. 
|===


== vérifiez -stats et vérifiez -csv

Utilisez le `-stats` et `-csv` paramètres avec le `verify` pour analyser les arborescences source et cible en parallèle et comparer les statistiques de l'arborescence.
...
Utilisez le `-nodata` paramètre avec le `verify` commande permettant de spécifier de ne pas vérifier les données.
...
Utilisez le `-noattrs` paramètre avec le `verify` commande permettant de spécifier de ne pas vérifier les attributs.
...
Utilisez le `-nomods` paramètre avec le `verify` commande permettant de spécifier de ne pas vérifier les heures de modification du fichier.
...
== vérifier -mtimewindow <s>

Utilisez le `-mtimewindow <s>` paramètre avec le `verify` pour spécifier la différence de temps de modification acceptable pour la vérification.
...
== vérifier -v et vérifier -l.

Utilisez le `-v` et `l` paramètres avec le `verify` pour récupérer les formats de sortie et répertorier les différences trouvées.
...
Utilisez le `-nonames` paramètre avec le `verify` commande permettant d'exclure les noms d'utilisateur et de groupe des listes de fichiers ou des rapports
...
Utilisez le `-match <filter>` paramètre avec le `verify` pour traiter uniquement les fichiers et répertoires correspondant au filtre.
...
Utilisez le `-bs <n>` paramètre avec le `verify` pour spécifier la taille des blocs de lecture/écriture. La valeur par défaut est 64k.
...
Utilisez le `-parallel <n>` paramètre avec le `verify` pour spécifier le nombre maximal de traitements par lots simultanés.
...
== vérifier -dircount <n[k]>

Utilisez le `-dircount <n[k]>` paramètre avec le `verify` commande permettant de spécifier la taille de la demande lors de la lecture des répertoires. La valeur par défaut est 64k.
...
Utilisez le `-noId` paramètre avec le `verify` pour désactiver la création d'un index par défaut. La valeur par défaut est FALSE.
...
Utilisez le `-preserve-atime` paramètre avec le `verify` pour restaurer tous les fichiers à la date de dernier accès sur la source. Le `-preserve-atime` Le paramètre réinitialise le temps d'accès à la valeur d'origine définie avant l'opération de lecture XCP.
...
== vérifier -s3.insecure

Utilisez le `-s3.insecure` paramètre avec le `verify` Commande permettant d'utiliser HTTP au lieu de HTTPS pour les communications entre les compartiments S3.
...
== vérifiez -s3.endpoint <s3_endpoint_url>

Utilisez le `-s3.endpoint <s3_endpoint_url>` paramètre avec le `verify` Commande permettant de remplacer l'URL du terminal AWS par défaut par une URL spécifiée pour la communication du compartiment S3.
...
== vérifier -s3.profile <name>

Utilisez le `s3.profile` paramètre avec le `verify` Commande permettant de spécifier un profil à partir du fichier d'informations d'identification AWS pour les communications du compartiment S3
...
== vérifier -s3.noverify

Utilisez le `-s3.noverify` paramètre avec le `verify` Commande permettant de remplacer la vérification par défaut de la certification SSL pour les communications de compartiment S3.
