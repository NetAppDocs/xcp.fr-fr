...
NOTE: L'option -stats imprime un rapport lisible sur la console. Les autres options de format de rapport sont -html ou -csv. Le format CSV (valeurs séparées par des virgules) possède des valeurs exactes. Si un catalogue existe, les rapports CSV et HTML sont automatiquement enregistrés dans le catalogue.
